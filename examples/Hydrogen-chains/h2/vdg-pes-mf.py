...
if __name__ == '__main__':
    ...
    for j, bs in enumerate(box_sizes):
        ...
        for i, bd in enumerate(bonds):
            ...
            # DG calculations
            print("Creating  DG Hamiltonianis ...")
            log.write("    Creating DG Hamiltonianis ...\n")
            start_H = time.time()

            log.write("        DZP-DG Hamiltonian ... \n")
            start = time.time()
            cell_dg_dzp  = dg.dg_model_ham(cell_dzp)#, None ,'rel_num', 0.99, True, voronoi_cells)
            log.write("        Done! Elapsed time: " + str(time.time() - start) + "sec.\n")
           
            log.write("        TZP-DG Hamiltonian ... \n")
            start = time.time()
            cell_dg_tzp  = dg.dg_model_ham(cell_tzp)#, None ,'rel_num', 0.99, True, voronoi_cells)
            log.write("        Done! Elapsed time: " + str(time.time() - start) + "sec.\n")

            log.write("        QZP-DG Hamiltonian ... \n")
            start = time.time()
            cell_dg_qzp  = dg.dg_model_ham(cell_qzp)#, None ,'rel_num', 0.99, True, voronoi_cells)
            log.write("        Done! Elapsed time: " + str(time.time() - start) + "sec.\n")
