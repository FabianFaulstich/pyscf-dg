...
if __name__ == '__main__':
    ...
    dgrid = [4]*3
    bonds = np.array([[10e-1]])
    bonds_max = np.max(bonds)
    offset = np.array([10., 10., 6.])
    X = np.array([int(np.ceil(offset[0] + bonds_max)), int(np.ceil(offset[1] + bonds_max)), offset[2]])
    ...
    for i, bd in enumerate(bonds):
        # atom position in xy-direction
        Z = (X[0]-np.sum(bd))/2.0 + np.append( 0, np.cumsum(bd))
        cell.atom = [[atom_spec, (Z[0], Z[0], X[2]/2)],
                     [atom_spec, (Z[0], Z[1], X[2]/2)],
                     [atom_spec, (Z[1], Z[0], X[2]/2)],
                     [atom_spec, (Z[1], Z[1], X[2]/2)]]
        cell.build()
